...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      obs_angle = normalize_angle(robot(3) + z(j).bearing);
      particles(i).landmarks(l).mu = robot(1:2) + [z(j).range * cos(obs_angle);
                                                   z(j).range * sin(obs_angle)];
      ...
      Q_inv = inv(Q);
      K = particles(i).landmarks(l).sigma * H' * Q_inv;
      ...
      particles(i).landmarks(l).mu = particles(i).landmarks(l).mu + K * zdiff;
      ...
      particles(i).weight =  particles(i).weight * sqrtm(det(2 * pi * Q)) * exp(-0.5 * zdiff' * Q_inv * zdiff);
    end
